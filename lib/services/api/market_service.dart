...
class MarketService {
  ...
  Future<CoinGeckoDetailModel?> fetchCoinDetail(
    ...
    final cacheBox = await _getCacheBox();
    final now = DateTime.now().millisecondsSinceEpoch;
    String dataKey = _generateCacheKey("detail_data",
        coinId: coinId, vsCurrency: vsCurrency);
    String timestampKey =
        _generateCacheKey("detail_ts", coinId: coinId, vsCurrency: vsCurrency);
        ...
        if (cacheAgeMinutes < _cacheDurationMinutesTTL) {
          ...
          if (cachedRawData != null) {
            try {
              ...
              print("Error parsing cached detail data: $e");
              await cacheBox.delete(dataKey);
              await cacheBox.delete(timestampKey);
            }
          }
    ...
    try {
      final dynamic responseData = await _networkService.get(
          '$_apiBaseUrl/coins/$coinId?localization=false&tickers=false&community_data=false&developer_data=false&sparkline=false');
      ...
      if (responseData is Map<String, dynamic>) {
        ...
        throw NetworkException('Invalid API response format for coin detail');
      }
    } catch (e) {
      print("Error fetching coin detail: $e");
      rethrow;
    }
  }
}
